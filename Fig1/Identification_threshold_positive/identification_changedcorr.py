"""DJF Niño 3.4 rainfall-correlation sensitivity analysis.

The script compares Pearson correlations between detrended DJF rainfall and
detrended DJF Niño 3.4 anomalies for a set of two-period splits. DJF Y is
defined as December Y-1 plus January-February Y.
"""
...
NINO34_PATH = Path("/Users/rizzie/ClimateData/climate_index/ENSO/nino34.anomersstv5.csv")
SCRIPT_DIR = Path(__file__).resolve().parent

FULLPERIOD_NC = SCRIPT_DIR / f"dcorr_nino34_djf_fullperiod_maps_{START_YEAR}_{END_YEAR}.nc"
SPLIT_NC = SCRIPT_DIR / f"dcorr_nino34_djf_split_maps_{START_YEAR}_{END_YEAR}.nc"
SPLIT_PERIODS_XLSX = SCRIPT_DIR / f"dcorr_nino34_djf_split_periods_{START_YEAR}_{END_YEAR}.xlsx"
SPLIT_METRICS_XLSX = SCRIPT_DIR / f"dcorr_nino34_djf_split_metrics_{START_YEAR}_{END_YEAR}.xlsx"
...
def linear_detrend(values):
    """Remove a linear trend along the first axis and preserve each mean."""
    values = np.asarray(values, dtype=float)
    x = np.arange(values.shape[0], dtype=float).reshape(
        (values.shape[0],) + (1,) * (values.ndim - 1)
    )
    valid = np.isfinite(values)
    count = valid.sum(axis=0)
    x_sum = np.where(valid, x, 0.0).sum(axis=0)
    y_sum = np.where(valid, values, 0.0).sum(axis=0)
    xx_sum = np.where(valid, x * x, 0.0).sum(axis=0)
    xy_sum = np.where(valid, x * values, 0.0).sum(axis=0)
    with np.errstate(divide="ignore", invalid="ignore"):
        slope = (count * xy_sum - x_sum * y_sum) / (count * xx_sum - x_sum**2)
        intercept = (y_sum - slope * x_sum) / count
        mean = y_sum / count
    trend = slope * x + intercept
    detrended = values - trend + mean
    detrended = np.where(count < 2, np.nan, detrended)
    return np.where(valid, detrended, np.nan)
...
rainfall_detrended = xr.DataArray(
    linear_detrend(rainfall_anomaly.values),
    coords=rainfall_anomaly.coords,
    dims=rainfall_anomaly.dims,
    name="djf_rainfall_anomaly_detrended",
)
nino34_detrended = pd.Series(
    linear_detrend(nino34_anomaly.values),
    index=nino34_anomaly.index,
    name="nino34_anomaly_detrended",
)
...
longitude_grid, latitude_grid = np.meshgrid(
    rainfall_detrended.lon.values,
    rainfall_detrended.lat.values,
)
land_mask = contains_xy(land_geometry, longitude_grid, latitude_grid)
valid_land_mask = land_mask & np.all(np.isfinite(rainfall_detrended.values), axis=0)

fullperiod_correlation = pearson_correlation_map(
    rainfall_detrended.values,
    nino34_detrended.values,
)
xr.Dataset(
    {"correlation": (("lat", "lon"), fullperiod_correlation)},
    coords={"lat": rainfall_detrended.lat, "lon": rainfall_detrended.lon},
    attrs={
        ...
        "detrending": "linear detrending applied to rainfall and Niño 3.4 anomalies",
    },
...
corr_p1 = np.full(
    (len(split_periods), rainfall_detrended.sizes["lat"], rainfall_detrended.sizes["lon"]),
    np.nan,
)
...
for split_number, split in split_periods.iterrows():
    ...
    p1_correlation = pearson_correlation_map(
        rainfall_detrended.sel(year=p1_years).values,
        nino34_detrended.loc[p1_years].values,
    )
    p2_correlation = pearson_correlation_map(
        rainfall_detrended.sel(year=p2_years).values,
        nino34_detrended.loc[p2_years].values,
    )
...
split_maps = xr.Dataset(
    ...
    coords={
        ...
        "lat": rainfall_detrended.lat,
        "lon": rainfall_detrended.lon,
    },
    attrs={
        "correlation": "Pearson correlation",
